...
int main() {
    ...
    while (1) {
        ...
        ssize_t request_len = recv(connected_fd, recv_buf, BUFFER_SIZE - 1, 0); // Use 0 flag instead of MSG_WAITALL
        if (request_len == -1) {
            ...
            recv_buf[request_len] = '\0'; // Null-terminate the received string
